...
* **Build queries visually with the PPL query builder:** A new visual query builder lets you construct PPL queries by selecting filters, aggregations, and sort criteria from menus, with seamless toggling to the raw code editor when you need full control.
* **Orient to your data with the Explore logs onboarding canvas:** Per-index cards show severity histograms and live log-line previews at a glance, guiding you through turning raw indexes into durable, reusable datasets.
* **Shape and compare data with new PPL commands:** Four new commands — makeresults, foreach, xyseries, and timewrap — enrich in-query data shaping and enable period-over-period comparisons such as day-over-day and week-over-week.
* **Catch query mistakes before they cost you a run:** A new PPL lint engine validates queries as you type, flagging unknown field references with one-click replacements and turning misspelled commands into suggestions.
* **Test grok patterns without leaving OpenSearch Dashboards:** A new Grok Debugger in Dev Tools lets you build and validate Grok patterns by pasting a sample log line and simulating the extraction to see parsed fields instantly.
* **Create alert rules directly from metric exploration:** A new "Create alert rule" action lets you go from identifying a metric pattern to alerting on it without leaving the page.
* **Load dashboards faster with lazy panel loading:** Dashboard panels located below the fold now defer data fetches until they scroll into the viewport, reducing concurrent query pressure on clusters with many-panel dashboards.

#### Infrastructure Updates

* **Deprecating support for Amazon Linux 2:** OpenSearch will deprecate support for Amazon Linux 2 as a build image and supported operating system in a future version, as Amazon Linux 2 reached end-of-life as of June 30, 2026.

### Experimental Features
OpenSearch 3.8 includes the following experimental functionality. Experimental features are disabled by default. For instructions on how to enable them, refer to the documentation for the feature.

* **Control storage growth with retention policies for agentic memory:** Configurable retention policies automatically delete expired sessions, long-term memories, and history entries based on time or count limits, with support for cluster-wide defaults or per-container configuration.
* **Query logs with SQL in Discover logs:** Discover logs now supports SQL queries, with full date picker integration and support across Logs, Visualization, and Statistics tabs.

## Release Details
[OpenSearch and OpenSearch Dashboards 3.8.0](https://opensearch.org/artifacts/by-version/#release-3-8-0) includes the following features, enhancements, bug fixes, infrastructure, documentation, maintenance and refactoring updates.

OpenSearch [Release Notes](https://github.com/opensearch-project/OpenSearch/blob/main/release-notes/opensearch.release-notes-3.8.0.md).

OpenSearch Dashboards [Release Notes](https://github.com/opensearch-project/OpenSearch-Dashboards/blob/main/release-notes/opensearch-dashboards.release-notes-3.8.0.md).
