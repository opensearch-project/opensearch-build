...
'''
This class is responsible for executing the build for a component and passing the results to an output recorder.
It will notify the output recorder of build information such as repository and git ref, and any artifacts generated by the build.
Artifacts found in "<build root>/artifacts/<maven|plugins|libs>" will be recognized and recorded.
'''
class Builder:
    def __init__(self, component_name, git_repo, component_scripts_path, default_build_script, output):
        '''
        ...
        :param output: The output recorder that will capture build information and artifacts.
        '''
    ...
    def build(self, version, arch):
        ...
        self.output.record_component(self.component_name, self.git_repo)
        ...
        for artifact_type in ["maven", "bundle", "plugins", "libs"]:
            for dir, dirs, files in os.walk(os.path.join(artifacts_dir, artifact_type)):
                for file_name in files:
                    ...
                    self.output.record_artifact(self.component_name, artifact_type, relative_path, absolute_path)
