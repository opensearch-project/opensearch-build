...
from aws.s3_bucket import STSError, S3DownloadError
...
class TestS3Bucket(unittest.TestCase):
    ...
    def test_s3_bucket_obj_sts_error(self, mock_boto_client):
        expected_sts_response = MockSTSResponse.successful_response()
        mock_boto_client('sts').assume_role.side_effect = ClientError(error_response={'Error': {'Code':'403'}},
    ...
    def test_upload_file(self, mock_boto_client):
        ...
        mock_boto_client('s3').upload_file.assert_called_once_with('/tmp/opensearch-1.1.0-linux-x64.tar.gz', bucket_name, 'tests/1.1.0/x64/opensearch-1.1.0-linux-x64.tar.gz')
    ...
    def test_download_file_failure(self, mock_boto_resource, mock_boto_client):
        ...
        file_path = 'tests/1.1.0/x64/opensearch-1.1.0-linux-x64.tar.gz'
        mock_boto_resource('s3').Bucket(bucket_name).download_file.side_effect = \
            ClientError(error_response={'Error': {'Code':'403'}},
