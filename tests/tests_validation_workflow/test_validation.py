...
class TestValidation(unittest.TestCase):
    ...
    def test_download_and_install_native_plugin_zip_exception(self, mock_temporary_directory: Mock, mock_file: Mock, mock_validation_args: Mock, mock_execute: Mock, mock_get: Mock) -> None:
        ...
        self.assertEqual(str(e.exception),
                         f"Unable to download and install native plugin: Command '.{os.sep}opensearch-plugin install"
                         f" --batch discovery-ec2', tmp{os.sep}trytytyuit{os.sep}bin' returned non-zero exit status 1.")
    ...
    def test_download_and_install_native_plugin_zip(self, mock_temporary_directory: Mock, mock_file: Mock,
        ...
        mock_execute.assert_has_calls(
            [
                call(f'.{os.sep}opensearch-plugin install --batch file:{os.path.join("tmp", "trytytyuit", "opensearch", "bin", "analysis-icu-3.0.0.zip")}',
                     os.path.join("tmp", "trytytyuit", "opensearch", "bin")),
                call(f'.{os.sep}opensearch-plugin install --batch file:{os.path.join("tmp", "trytytyuit", "opensearch", "bin", "analysis-nori-3.0.0.zip")}',
                     os.path.join("tmp", "trytytyuit", "opensearch", "bin"))]
        )
