...
class TestBenchmarkTestCluster(unittest.TestCase):
    ...
    @patch("subprocess.run")
    @patch("requests.get")
    @patch('test_workflow.benchmark_test.benchmark_test_cluster.AWS4Auth')
    @patch('test_workflow.benchmark_test.benchmark_test_cluster.os.getenv')
    def test_endpoint_with_sigv4(self, mock_getenv: Mock, mock_aws4auth: Mock, mock_requests_get: Mock, mock_subprocess_run: Mock) -> None:
        self.args.sigv4 = True
        self.args.region = "us-west-2"
        self.args.service = "es"
        ...
        with patch("json.loads"):
            self.benchmark_test_cluster.start()
            mock_requests_get.assert_called_with(url=f"https://{self.benchmark_test_cluster.endpoint}/_cluster/health", auth=mock_aws4auth.return_value, verify=False)
            mock_aws4auth.assert_called_with('test_access_key', 'test_secret_key', 'us-west-2', 'es')

    @patch("subprocess.run")
    @patch("requests.get")
    def test_endpoint_with_serverless_404(self, mock_requests_get: Mock, mock_subprocess_run: Mock) -> None:
        mock_result = MagicMock()
        mock_result.stdout = '''
        {
            "cluster_name" : "opensearch-cluster.amazon.com",
        ...
        with patch("json.loads"):
            with patch("logging.info") as mock_logging:
                self.benchmark_test_cluster.start()
                mock_logging.assert_any_call("404 response indicates serverless collection, skipping health checks")

    @patch("subprocess.run")
    def test_start_with_sigv4_command(self, mock_subprocess_run: Mock) -> None:
        self.args.sigv4 = True
        mock_result = MagicMock()
        mock_result.stdout = '''
        {
        ...
        with patch("json.loads"):
            with patch.object(self.benchmark_test_cluster, 'wait_for_processing'):
                self.benchmark_test_cluster.start()
                mock_subprocess_run.assert_called_with(
                    f"curl https://{self.benchmark_test_cluster.cluster_endpoint}",
