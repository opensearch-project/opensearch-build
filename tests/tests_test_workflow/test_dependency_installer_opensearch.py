...
class DependencyInstallerOpenSearchTests(unittest.TestCase):
    ...
    def test_install_maven_dependencies_local(self, mock_request: MagicMock, mock_copyfile: MagicMock, mock_makedirs: MagicMock) -> None:
    ...
    @patch("shutil.copyfile")
    @patch("urllib.request.urlretrieve")
    def test_install_maven_dependencies_remote(self, mock_request: MagicMock, mock_copyfile: MagicMock, mock_makedirs: MagicMock) -> None:
        counter = ThreadSafeCounter()
    ...
    @patch("shutil.copyfile")
    @patch("urllib.request.urlretrieve")
    def test_install_maven_dependencies_remote_failure(self, mock_request: MagicMock, mock_copyfile: MagicMock, mock_makedirs: MagicMock) -> None:
        def mock_retrieve(source: Any, dest: Any) -> None:
            raise HTTPError(url=source, hdrs=Message(), fp=None, msg="Not Found", code=404)
    ...
    @patch("shutil.copyfile")
    @patch("urllib.request.urlretrieve")
    def test_install_build_dependencies_local(self, mock_request: MagicMock, mock_copyfile: MagicMock, mock_makedirs: MagicMock) -> None:
        dependency_installer = DependencyInstallerOpenSearch(
            self.DATA,
            BuildManifest.from_path(self.BUILD_MANIFEST),
            BundleManifest.from_path(self.DIST_MANIFEST_LOCAL)
    ...
    @patch("shutil.copyfile")
    @patch("urllib.request.urlretrieve")
    def test_install_build_dependencies_remote(self, mock_request: MagicMock, mock_copyfile: MagicMock, mock_makedirs: MagicMock) -> None:
        dependency_installer = DependencyInstallerOpenSearch(
