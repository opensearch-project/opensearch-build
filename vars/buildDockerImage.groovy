void call(Map args = [:]) {
    ...
    def x64Response = args.dryRun ? [status: 200] : httpRequest(httpMode: 'HEAD', url: x64TarGz)
    def arm64Response = args.dryRun ? [status: 200] : httpRequest(httpMode: 'HEAD', url: arm64TarGz)

    if (x64Response.status != 200 || arm64Response.status != 200) {
        echo([
            'Skipping docker build.',
            "${x64TarGz} (${x64Response.status})",
            "${arm64TarGz} (${arm64Response.status})"
        ].join('\n'))
    } else {
        def parameters = [
