...
COPY * $TEMP_DIR
RUN tar -xzpf /tmp/opensearch-dashboards/opensearch-dashboards-`uname -p`.tgz -C $OPENSEARCH_DASHBOARDS_HOME --strip-components=1 && \
    ...
    ls -l $OPENSEARCH_DASHBOARDS_HOME && rm -rf $TEMP_DIR
